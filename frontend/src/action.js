...
export const requestRMVTodo = (id, csrftoken) => (dispatch) => {
    dispatch({ type: REQUEST_REMOVE_TODO_PENDING });
    ...
    .then((res) => res.json())
    .then((data) => dispatch({ type: REQUEST_REMOVE_TODO_SUCCESS, payload: {id, data} }))
    .catch((err) => dispatch({ type: REQUEST_REMOVE_TODO_FAILED, payload: err }));
};
